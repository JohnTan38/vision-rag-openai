...
// ✅ AGGRESSIVE FIX: Further reduced limits
const MAX_IMAGE_PAGES = 2;  // Reduced from 3 to 2
const IMAGE_SCALE = 0.4;    // Reduced from 1.0 to 0.4 (60% reduction in pixels)
const JPEG_QUALITY = 0.4;   // Reduced from 0.7 to 0.4 (better compression)
const MAX_PAYLOAD_SIZE_MB = 3.5;  // More conservative limit
const MAX_IMAGE_WIDTH = 1200;     // NEW: Maximum width constraint
const MAX_IMAGE_HEIGHT = 1600;    // NEW: Maximum height constraint
...
export default function Home() {
  ...
  const [allPdfImages, setAllPdfImages] = useState<string[]>([]); // NEW: Store all rendered pages
  ...
  // ✅ ENHANCED: Render with dimension constraints and better compression
  const renderPdfToImages = async (file: File, maxPages: number = MAX_IMAGE_PAGES): Promise<string[]> => {
    ...
    for (let pageIndex = 1; pageIndex <= pagesToRender; pageIndex += 1) {
      ...
      // ✅ NEW: Apply dimension constraints
      ...
      // ✅ AGGRESSIVE COMPRESSION: JPEG at 40% quality
      const imageDataUrl = canvas.toDataURL('image/jpeg', JPEG_QUALITY);
      
      // Log individual image size for debugging
  ...
  // ✅ NEW: Intelligent page selection based on query
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && file.type === 'application/pdf') {
      ...
      try {
        ...
        try {
          // ✅ Render up to 3 pages initially, but only use 2 per query
        ...
        setPdfFile(file);
        setPdfBase64(base64);
        setAllPdfImages(renderedImages); // Store all rendered pages
        setPdfImages(renderedImages.slice(0, 2)); // Initially use first 2
  ...
  const handleQuery = async (queryText: string = input) => {
    ...
    // ✅ NEW: Select relevant pages based on query
  ...
  return (
    <div className="flex h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 text-slate-900 font-sans overflow-hidden">
            ...
            {pdfFile ? (
              <div className="space-y-2">
                ...
                <div className="text-xs text-slate-500 space-y-1">
                  <div>📄 {allPdfImages.length} page(s) available</div>
                  <div>🎯 Smart selection active</div>
                  <div>📊 Quality: {(JPEG_QUALITY * 100).toFixed(0)}%</div>
                </div>
                <button
                  onClick={clearChat}
      ...
      {/* Main Content - keeping the same UI as before */}
              ...
              {messages.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center max-w-3xl mx-auto space-y-8 animate-fade-in">
                  ...
                  <div className="space-y-4">
                    ...
                    <p className="text-sm text-slate-500 bg-blue-50 p-3 rounded-lg inline-block">
                      ✨ Optimized for large PDFs with smart page selection
                    </p>
                  </div>
                    ...
                    <div className="relative border-2 border-dashed border-purple-300 group-hover:border-purple-500 rounded-3xl p-12 bg-white/80 backdrop-blur-sm transition-all flex flex-col items-center justify-center space-y-4 shadow-lg group-hover:shadow-2xl group-hover:scale-105">
                      ...
                      <div className="relative text-center">
                        ...
                        <p className="text-sm text-slate-500 mt-2">
                          Files up to 50MB supported • Optimized compression
                        </p>
                      </div>
                    </div>
                    <input
                ...
                <div className="flex justify-start animate-fade-in">
                  <div className="bg-white/90 backdrop-blur-sm border border-purple-200 rounded-2xl rounded-tl-none px-6 py-4 flex items-center gap-3 shadow-lg">
                    ...
                    <span className="text-slate-600 text-sm">
                      Analyzing document with smart page selection...
                    </span>
                  </div>
                </div>
              )}
            ...
            <div className="p-6 bg-gradient-to-br from-blue-50 to-purple-50 border border-purple-200 rounded-3xl shadow-lg">
              ...
              <ul className="text-sm text-purple-800 space-y-2.5">
                <li className="flex items-start gap-2">
                  ...
                  />
                  <span>
                    <strong>Smart Page Selection:</strong> Automatically selects relevant pages based on your query
                  </span>
                </li>
                <li className="flex items-start gap-2">
                  <ChevronRight
                  ...
                  />
                  <span>
                    <strong>Aggressive Compression:</strong> {(JPEG_QUALITY * 100).toFixed(0)}% JPEG quality at {IMAGE_SCALE}x scale
                  </span>
                </li>
                <li className="flex items-start gap-2">
                  <ChevronRight
                  ...
                  />
                  <span>
                    <strong>Dimension Limits:</strong> Max {MAX_IMAGE_WIDTH}x{MAX_IMAGE_HEIGHT}px per page
                  </span>
                </li>
                <li className="flex items-start gap-2">
                  <ChevronRight
                  ...
                  />
                  <span>
                    <strong>Fallback System:</strong> Auto-reduces to 1 page if payload still too large
                  </span>
                </li>
              </ul>
            </div>
